# Bulletproof+ range proof with double-value commitments
#
# {(H,H_,G,n),C ; (v,a,r) | 0 <= v < 2^n, C = vH + aH_ + rG}
...
class RangeParameters:
	def __init__(self,H,H_,G,N):
		if not isinstance(H,Point):
			raise TypeError('Bad type for parameter H!')
		if not isinstance(H_,Point):
			raise TypeError('Bad type for parameter H1!')
...
class RangeWitness:
	def __init__(self,v,a,r):
		...
		if not isinstance(a,ScalarVector):
			raise TypeError('Bad type for range witness a!')
...
class RangeProof:
	...
	def __init__(self,A,A1,B,r1,s1,d1,d1_,L,R):
		...
		if not isinstance(d1_,Scalar):
			raise TypeError('Bad type for range proof element d1_!')
...
class InnerProductRound:
	def __init__(self,Gi,Hi,G,H,H_,a,b,alpha,alpha_,y,tr):
		# Common data
		self.Gi = Gi
		self.Hi = Hi
		self.G = G
...
def inner_product(data):
	...
	if n == 1:
		...
		data.A = data.Gi[0]*r + data.Hi[0]*s + data.H*(r*data.y*data.b[0] + s*data.y*data.a[0]) + data.G*d + data.H_*d_
		data.B = data.H*(r*data.y*s) + data.G*eta + data.H_*eta_
		...
		data.d1_ = eta_ + d_*e + data.alpha_*e**2
	...
	data.L.append(G2**(a1*data.y.invert()**n) + H1**b2 + data.H*cL + data.G*dL + data.H_*dL_)
	data.R.append(G1**(a2*data.y**n) + H2**b1 + data.H*cR + data.G*dR + data.H_*dR_)
	...
	data.alpha_ = dL_*e**2 + data.alpha_ + dR_*e.invert()**2
...
def prove(statement,witness):
	...
	m = len(statement.C)
	for j in range(m):
		if not statement.C[j] == witness.v[j]*statement.H + witness.a[j]*statement.H_ + witness.r[j]*statement.G:
	...
	tr = transcript.Transcript('Double Bulletproof+')
	tr.update(G)
	tr.update(H)
	tr.update(H_)
	tr.update(N)
	...
	alpha = random_scalar()
	alpha_ = random_scalar()
	A = Gi**aL + Hi**aR + G*alpha + H_*alpha_
	...
	for j in range(M):
		...
		alpha1_ += z**(2*(j+1))*witness.a[j]*y**(M*N+1)

	# Initial inner product inputs
	ip_data = InnerProductRound(Gi,Hi,G,H,H_,aL1,aR1,alpha1,alpha1_,y,tr)
	while True:
		inner_product(ip_data)

		# We have reached the end of the recursion
		if ip_data.done:
			return RangeProof(A,ip_data.A,ip_data.B,ip_data.r1,ip_data.s1,ip_data.d1,ip_data.d1_,ip_data.L,ip_data.R)
...
def verify(statements,proofs):
	...
	for statement in statements:
		...
		if H_ is not None and statement.H_ != H_:
			raise ValueError('Inconsistent range batch statements!')
		else:
			H_ = statement.H_
